...
def index(request):
    message=''
    if(models.User.objects.filter(user_id=request.user)):
        user=models.User.objects.get(user_id=request.user)
    ...
    if form.is_valid():
        ...
        if User.qbank_set.filter(qbank_id=qb):
            ...
            if not models.Test.objects.filter(test_id=test_id):
                qb_select=User.qbank_set.get(qbank_id=qb)
                workbook=xlrd.open_workbook('D:\\Smart-Examination-master\\MyPrj\\'+qb_select.qbank_file.url)
                worksheet=workbook.sheet_by_index(0)
                ...
                if(mark3<=len(mark3qnos) or mark4<=len(mark4qnos) or mark7<=len(mark7qnos)):
                    ...
                    test_file=xlwt.Workbook()
                    # test_file.save('D:\\Smart-Examination-master\\MyPrj\\tests\\'+test_id+'.xls')
                    sheet=test_file.add_sheet('sheet_1',cell_overwrite_ok=True)
                    sheet.write(0,0,"qno")
